...
public interface ReservaRepository extends JpaRepository<Reserva, Integer> {
        ...
        @Query(value = "SELECT COUNT(*) > 0 FROM reserva r WHERE r.codigo_bicicleta = :codigoBicicleta " +
                        "AND r.fecha_reserva = :fecha AND r.estado_reserva IN ('ACTIVA', 'CONFIRMADA') " +
                        "AND ((r.hora_reserva <= :horaInicio AND ADDTIME(r.hora_reserva, SEC_TO_TIME(:duracionHoras * 3600)) > :horaInicio) "
                        +
                        "OR (r.hora_reserva < ADDTIME(:horaInicio, SEC_TO_TIME(:duracionHoras * 3600)) AND r.hora_reserva >= :horaInicio))", nativeQuery = true)
        boolean existeConflictoReserva(@Param("codigoBicicleta") String codigoBicicleta,
        ...
        List<Reserva> findReservasByFechaRange(@Param("fechaInicio") Date fechaInicio,
